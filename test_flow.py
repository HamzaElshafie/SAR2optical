...
def get_sample_files(classpath):
    # classpath: ex: images/train, which includes different classes samples with each class in its folder
...
def data_generator():
    ...
    sar_path = os.path.join(rootpath, 'content/SAR2optical/dataset/train/s1')
    optical_path = os.path.join(rootpath, 'content/SAR2optical/dataset/train/s2')

    save_dir_rgb = os.path.join(rootpath, 'content/SAR2optical/dataset/train/rgb')
    save_dir_gray = os.path.join(rootpath, 'content/SAR2optical/dataset/train/gray')
    ...
    train_image_generator = train_data_gen.flow_from_directory(directory=sar_path, target_size=(256, 256), 
                                                               color_mode='grayscale', save_to_dir=save_dir_gray, 
                                                               class_mode=None, batch_size=1, shuffle=True, seed=seed)
    train_label_generator = train_data_gen.flow_from_directory(directory=optical_path, target_size=(256, 256), 
                                                               color_mode='rgb', save_to_dir=save_dir_rgb, 
                                                               class_mode=None, batch_size=1, shuffle=True, seed=seed)
    
    return train_image_generator, train_label_generator

def test_generator():
    sar_val_path = "/content/SAR2optical/dataset/test/s1"
    optical_val_path = "/content/SAR2optical/dataset/test/s2"
    
    test_data_gen = ImageDataGenerator(
        rescale=1./255
    )
    seed = 14
    val_image_generator = test_data_gen.flow_from_directory(directory=sar_val_path, target_size=(256, 256), 
                                                            color_mode='grayscale', class_mode=None, 
                                                            batch_size=32, shuffle=True, seed=seed)
    val_label_generator = test_data_gen.flow_from_directory(directory=optical_val_path, target_size=(256, 256), 
                                                            color_mode='rgb', class_mode=None, batch_size=32, 
